...
- владение результатами обработки, например агрегированными строками Waterfall, snapshot-данными и входными кадрами пеленгации;
- контракты data plane: `SignalBlock`, memory pool, bounded queue, backpressure, метрики задержек и пропускной способности;
...
- публичные контракты ingest pipeline, DSP pipeline, storage pipeline и snapshot publisher, когда они появляются;
...
- комментарии processing/data plane должны описывать независимые от пользовательского интерфейса входы, выходы, владение блоками, backpressure и допустимые потоки вызовов;
...
Комментарии не должны представлять `WaterfallController`, `SignalSampleBus`,
`BearingFrameBus`, `QObject` или Qt queued signals как допустимый high-load raw stream
transport. Если такой путь упоминается, его нужно явно обозначать как current/legacy или
